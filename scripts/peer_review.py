...
def _main(argv=None):
    ...
    with tempfile.TemporaryDirectory() as work_dir:
        ...
        if args.mode == "spec":
            ...
            prompt_lines.append("5. Authoritative contracts: Are external contracts/schemas authoritative and consistent, with clear separation of observable contracts from internal implementation details?")
            ...
            if args.spec:
                ...
                prompt_lines.append("Focus on clearly bounded operational task scoping, requirement traceability (tasks linking to REQ-xxx and declaring dependencies), TDD rigor, explicit interface contracts (Consumes/Produces), exact test commands with assertions, and complete zero-placeholder diff implementations.")
            else:
