...
- Generate a comprehensive diff saving to a unique absolute path:
  ```bash
  mkdir -p .tribunal
  if git rev-parse HEAD >/dev/null 2>&1; then git diff --cached HEAD > .tribunal/current_review.diff; else git diff --cached 4b825dc642cb6eb9a060e54bf8d69288fbee4904 > .tribunal/current_review.diff; fi
  ```

## Phase 2: Invocation (Model B)
- Run: `python3 ~/.gemini/config/plugins/ai-review-plugin/scripts/peer_review.py --mode code --target "$(pwd)/.tribunal/current_review.diff" --repo "$(pwd)"`
...
- **Exit 1**: Fix code or formulate a rebuttal, STAGE the fixes with `git add <files>` (if any), REGENERATE the diff to the SAME `.tribunal/current_review.diff`, retry passing `--session-id <SESSION_ID>` and optionally `--message`.
